...
将 `/Users/zhizhidemac/Documents/Obsidian Vault` 作为所有项目的长期记忆来源，并遵循全局 Skill：`~/.claude/skills/obsidian-project-memory/SKILL.md`。

1. **相关任务开始前检索**：项目开发/排障、历史决策或依赖已有偏好的任务，按项目绝对路径、精确项目名和一至两个唯一业务词定向检索；只读相关章节，无命中不扩大为全库读取。独立翻译、改写、简单概念问答和无关网页阅读不强制检索；涉及既有项目或历史背景时仍需检索。
2. **历史笔记必须复核**：笔记用于补充上下文，不替代实时事实。分支、工作区、版本、接口、部署、运行状态等易变信息必须在当前环境重新验证。
3. **重要任务完成后必须回写**：在最终回复前，将新增且未来有价值的项目背景、关键决策、解决方案、代码或配置变更、验证边界、当前进度、失败根因、可复用方法和下一步合并到对应笔记。
4. **优先更新，避免膨胀**：写入前先搜索，优先更新 `10-Projects` 中已有的一页式项目记忆；仅当结论确实可跨项目复用时，才提升到 `20-Knowledge` 或 `30-Lessons`。不记录寒暄、推理草稿、重复结论、一次性命令输出或可随时从代码直接发现的普通事实。
5. **敏感信息绝不入库**：不得为了生成笔记读取 `.env` 或凭证文件；不得记录密码、API Key、Token、Cookie、Session、私钥、助记词或含凭证的连接串。只允许记录脱敏位置、凭证类型和处置状态。
6. **回写不扩大权限**：更新 Obsidian 不代表获准提交、推送、合并、部署、发送外部消息或修改任务范围之外的内容。
7. **最终回复说明结果**：执行记忆流程时列出更新笔记及经验类别；无需记忆的普通问答不附记忆报告。
