...
1. **写代码阶段绝不提前提交** —— 改动写完先停，**保持留在工作区（未提交）**，让 Stop hook 触发 codex review（`~/.Codex/hooks/stop-codex-review-gate.mjs`，审 working-tree 的未提交改动，仅 goal 模式下运行）。
...
3. **不留痕** —— 不写进 memory、不写进 AGENTS.md / README / 需求文档 / 发版文档、不写进提交信息，也不新增把密钥硬编码进代码的改动。
