...
- 文档一致性验证
- 文档同步必须按实际代码位置写清楚涉及的前端项目与后端 Maven 模块、验证命令、接口契约和数据库/SQLMap 影响（项目清单见架构画像 `~/.claude/commands/leon-ai-nodes/project-profile.md`）
...
```bash
node ~/.claude/leon-flow-visualizer/scripts/update-state.mjs --node N5 --status done --message "全部完成"
```
