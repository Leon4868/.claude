# /leon:init — 项目 .claude 初始化

你是一个项目配置初始化助手。你的任务是在目标项目目录中创建或补齐 `.claude/` 文件夹及其完整配置结构。
...
- **当前目录初始化**：`/leon:init`
- **指定单项目初始化**：`/leon:init {项目路径}`
- **多工程路径初始化**：`/leon:init 前端{FRONTEND_ROOT} 后端{BACKEND_ROOT}`
- **公共根目录初始化**：`/leon:init --root {公共根目录}` 或用户明确说明“在公共根目录统一初始化”

如果未提供参数，默认以当前工作目录作为候选目标项目；如果无法唯一判断目标项目或初始化模式，必须暂停并要求用户确认，禁止猜测生成。

## 执行步骤

## 项目路径提示

初始化时从用户指定路径、当前工作目录或项目 manifest 推断代码根目录，并按角色记录 `FRONTEND_ROOT`（前端代码根目录）、`{BACKEND_ROOT}/ai`（后端 Maven 根目录）。前端多项目 / 后端 Maven 多模块的具体架构与技术栈见架构画像 `~/.claude/commands/leon-ai-nodes/project-profile.md`（初始化前读取）。

初始化时必须针对具体子项目或后端根工程生成 `.claude/`；不要把前端三个项目和后端多模块混成一个单体规则文件，除非用户明确要求在公共根目录统一初始化。禁止写死某台机器的绝对路径。

### 初始化目标选择

在进入第 0 步之前，先确定 `TARGET_DIR` 和初始化模式：

- 如果参数是单个项目路径，`TARGET_DIR` 为该路径。
- 如果参数包含 `前端{FRONTEND_ROOT}` 或 `后端{BACKEND_ROOT}`：
  - 对前端根下的具体项目分别初始化：`ai-admin-ui`、`ai-decision-system-ui`、`ai-seat-console`（只初始化实际存在的目录）。
  - 对后端 Maven 根工程 `{BACKEND_ROOT}/ai` 初始化一次，用于覆盖后端多模块公共规则。
  - 每个目标目录独立执行第 0～5 步，互不覆盖。
- 如果参数为 `--root {公共根目录}`，或用户明确要求公共根目录统一初始化：
  - 在公共根目录生成一个**总入口** `.claude/CLAUDE.md`，记录前端根、后端根、各子项目入口和通用约束。
  - 不在总入口里伪造单体技术栈；必须明确这是“多工程索引”。
  - 如子项目缺少 `.claude/`，在总结中建议继续对具体子项目运行 `/leon:init {子项目路径}`；除非用户明确要求批量初始化子项目，否则不要自动深入写入所有子项目。
- 如果当前目录同时像公共根目录又像具体项目，必须暂停确认目标模式。
...
- **已存在** → 进入「增量补充模式」，**核心原则是保留用户已有内容、只补齐缺失项，绝不覆盖或删除现有内容**。流程如下：
  ...
  1. **盘点现有配置**：递归读取 `.claude/` 下所有文件，重点是 `CLAUDE.md` 和 `rules/*.md`，搞清楚：
     ...
     - 是否存在用户自定义的、不在 leon:init 标准结构内的文件或章节（这些一律保留，不要动）。
  2. **分析项目本身**：执行第 1 步「分析项目」，得出项目当前真实的语言/框架/模块构成（前端、后端 API、数据库、App、合约等）。
  3. **计算差异（标准结构 − 现有内容）**：对照 leon:init 的标准结构（第 2 步的文件结构、第 3 步的 CLAUDE.md 章节、第 5 步的规则内容指引），找出：
  ...
  4. **执行补充（只增不改）**：
     ...
     - 追加前必须做去重检查：同名标题、同义标题（如「开发命令」≈「常用命令」）、已有 `@rules/xxx.md` 引入行都视为已存在，禁止重复追加；
     - 如果缺失章节需要插入到已有结构中，但无法在不重排文件的前提下自然追加，则只在文件末尾追加「补充配置」章节，并在总结里说明；
...
- 如果分析结果无法唯一确定语言、框架、包管理器、测试命令、目标子项目或后端模块边界，必须暂停确认；不要把不确定内容写入规则文件
...
│   ├── frontend.md              # (如有前端) paths 按真实源码目录推断
│   ├── backend-api.md           # (如有后端 API) paths 按真实模块目录推断
│   ├── database.md              # (如有数据库) paths 按真实 DAO/SQLMap/migration 目录推断
│   ├── app.md                   # (如有移动端/桌面 App) paths: android/**, ios/**, lib/**, native/**
│   └── smart-contract.md        # (如有合约) paths: contracts/**, src/contracts/**
```

`globs` 必须基于真实目录生成；若无法可靠推断，允许省略 `globs`，但不要写入与项目结构不匹配的示例路径。
...
公共根目录模式下，`CLAUDE.md` 必须使用“多工程索引”模板，不得套用单项目模板：

```markdown
# {公共根目录名}

多工程代码索引，包含前端多项目与后端 Maven 多模块工程。
...
- `ai-admin-ui`: {技术栈与常用命令摘要}
- `ai-decision-system-ui`: {技术栈与常用命令摘要}
- `ai-seat-console`: {技术栈与常用命令摘要}

## 后端工程

- Maven 根工程: `{BACKEND_ROOT}/ai`
- 主要模块: `ai-admin`, `ai-open-api`, `ai-server`, `ai-dataaccess`, ...

## 使用约定

- 需求、设计、任务中必须写清楚目标前端项目或后端 Maven 模块。
- 修改代码前先进入具体子项目/模块读取本地 `.claude/` 和项目配置。
- 禁止把所有前端和后端描述成单体应用。
```
